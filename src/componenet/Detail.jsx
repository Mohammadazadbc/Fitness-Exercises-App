...
function Detail({exerciseData}) {

    const { name, bodyPart, gifUrl, target, equipment } = exerciseData
    const extraDetail = [
        {
            icon: BodyPartImage,
            name: bodyPart
        ...
        <Stack sx={{gap:{lg:'35px', xs:'20px' }}} >
            ...
            {extraDetail.map((item)=>(
                <Stack key={item.name} direction="row" gap="24px" alignItems='center' >
                    <Button sx={{background: '#fff2db', borderRadius:'50%', width:'100px', height:'100px'}} > <img style={{width:'50px', height:'50px'}} src={item.icon} alt="" /> </Button>
